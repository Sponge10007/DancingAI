...
import { GLMService, VideoComparisonResult } from '../services/glmService';
...
export default function HomeScreen({ navigation }: HomeScreenProps) {
  ...
  const [comparisonResult, setComparisonResult] = useState<VideoComparisonResult | null>(null);
  ...
  const aiSummary = {
    title: '上次舞蹈练习整体进展顺利',
    content: '基本完成了动作组合的要求',
    suggestion: '节奏感和动作衔接相比之前有进步',
    note: '这次练习将更注重动作的稳定性与表现力',
  };
  ...
  const handlePlayPress = (card: any, event: any) => {
    ...
    console.log('视频信息:', {
      ...
      isTestVideo: card.videoUrl.includes('test_origin.mp4')
    });
    navigation.navigate('VideoDetail', {
      video: card,
  ...
  const useTestVideo = () => {
    // 使用本地测试文件路径 - 您跳的舞蹈视频
    const localTestPath = 'D:\\Files\\DancingAI\\frontend\\src\\assets\\test_mine.mp4';
  ...
  const startAnalysis = async () => {
    ...
    try {
      ...
      // 原版视频路径
      const originalVideoPath = 'D:\\Files\\DancingAI\\frontend\\src\\assets\\test_origin.mp4';

      // 调用GLM服务进行对比分析
      const glmService = GLMService.getInstance();
      const result = await glmService.compareVideos(uploadedVideo, originalVideoPath);

      setComparisonResult(result);

    } catch (error) {
      console.error('AI对比分析错误:', error);
  ...
  const convertVideoToBase64 = async (videoPath: string): Promise<string> => {
    try {
      ...
      // 检查是否是本地测试文件
      if (videoPath.includes('test_mine.mp4') || videoPath.includes('test_origin.mp4') || videoPath.includes('D:\\Files\\DancingAI')) {
      ...
      if (videoPath.includes('test_mine.mp4') || videoPath.includes('test_origin.mp4')) {
      ...
      >
        <View style={styles.compareModalOverlay}>
          <View style={styles.compareModalContent}>
            ...
            {uploadedVideo && !comparisonResult && !isAnalyzing && (
              <View style={styles.uploadSection}>
                <View style={styles.videoSelected}>
                  ...
                  <Text style={styles.videoInfo}>
                    您的视频: {uploadedVideo.length > 50 ? '...' + uploadedVideo.slice(-50) : uploadedVideo}
                  </Text>
                  <Text style={styles.videoInfo}>
                    对比视频: test_origin.mp4 (原版舞蹈)
                  </Text>
                </View>
                <View style={styles.buttonGroup}>
                  ...
                  <TouchableOpacity style={styles.analyzeButton} onPress={startAnalysis}>
                    <Text style={styles.analyzeButtonText}>开始AI对比</Text>
                  </TouchableOpacity>
                </View>
              </View>
            )}
            ...
            {isAnalyzing && (
              <View style={styles.analyzingSection}>
                ...
                <Text style={styles.analyzingText}>AI正在对比分析您的舞蹈...</Text>
                <Text style={styles.analyzingSubtext}>正在与原版视频进行智能对比，请稍候...</Text>
              </View>
            )}
            ...
            {comparisonResult && (
              ...
              >
                <View style={styles.resultSection}>
                  <Text style={styles.resultTitle}>AI对比分析结果</Text>
                  <View style={styles.scoreSection}>
                    <Text style={styles.scoreLabel}>综合评分</Text>
                    <Text style={styles.scoreValue}>{comparisonResult.score}/100</Text>
                  </View>

                  <View style={styles.feedbackSection}>
                    <Text style={styles.feedbackTitle}>📝 文字指导</Text>
                    {comparisonResult.textGuidance.map((item: string, index: number) => (
                      <Text key={index} style={styles.feedbackItem}>• {item}</Text>
                    ))}
                  </View>

                  <View style={styles.improvementSection}>
                    <Text style={styles.improvementTitle}>🎯 动作指导</Text>
                    {comparisonResult.actionGuidance.map((item: string, index: number) => (
                      <Text key={index} style={styles.improvementItem}>• {item}</Text>
                    ))}
                  </View>

                  <View style={styles.similaritiesSection}>
                    <Text style={styles.similaritiesTitle}>✅ 相似之处</Text>
                    {comparisonResult.similarities.map((item: string, index: number) => (
                      <Text key={index} style={styles.similarityItem}>• {item}</Text>
                    ))}
                  </View>

                  <View style={styles.differencesSection}>
                    <Text style={styles.differencesTitle}>🔄 需要改进</Text>
                    {comparisonResult.differences.map((item: string, index: number) => (
                      <Text key={index} style={styles.differenceItem}>• {item}</Text>
                    ))}
                  </View>
                  ...
                  <TouchableOpacity style={styles.newAnalysisButton} onPress={() => {
                    ...
                    <Text style={styles.newAnalysisButtonText}>对比新视频</Text>
                  </TouchableOpacity>
                </View>
              </ScrollView>
            )}
...
const styles = StyleSheet.create({
  ...
  resultScrollContent: {
    paddingBottom: 20, // 底部留白，确保最后内容可见
  },
  resultSection: {
    paddingVertical: 10,
  },
